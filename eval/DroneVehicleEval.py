...
def parse_gt(xml_path):
    ...
    for obj in root.findall('object'):
        ...
        if name == '*':
            continue
        if name in ['feright_car', 'feright_car', 'feright', 'freight car', 'freight_car']:
...
def voc_eval(detpath,
    ...
    for t in BB:
        ...
        t6 = t[0]
        t7 = t[1]
        newBB.append([t0, t1, t2, t3, t4, t5, t6, t7]) 
    # BB = np.array([[float(z) for z in x[2:6]] + [float(x[4]), float(x[3]), float(x[2]), float(x[5])] for x in splitlines])
    oldBB = BB
    BB = np.array(newBB)
    # sort by confidence
    ...
    for d in range(nd):
        ...
        if BBGT.size > 0:
            ...
            if len(BBGT_keep) > 0:
                ...
                overlaps = overlaps[BBGT_keep_index]
...
def main():
    detpath = r'/media/khmt/disk1/tungp/UAV_CV/AerialDetection/work_dirs/htc_without_semantic_r50_fpn_1x_dota1_5/Task2_results/{:s}.txt'
    # annopath = r'/media/khmt/disk1/tungp/UAV_CV/DroneVehicle/val/vallabel{:s}.xml'    # for eval val set
    annopath = r'/media/khmt/disk1/tungp/UAV_CV/DroneVehicle/test/testlabelr/{:s}.xml'     # for eval test set

    # imagesetfile = r'root/UA-CMDet/filename_val.txt'      # for eval val set
    imagesetfile = r'/media/khmt/disk1/tungp/UAV_CV/UA-CMDet/filename_test.txt'       # for eval test set
